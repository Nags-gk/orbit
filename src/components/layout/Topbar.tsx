...
export function Topbar({ onMenuClick }: TopbarProps) {
    ...
    // Fallback to "User" if name is not set, extract initials
            ...
            <div className="ml-auto flex items-center gap-3">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className="p-2 rounded-full hover:bg-foreground/10 text-muted-foreground hover:text-foreground transition-colors relative outline-none">
                            <Bell className="w-5 h-5" />
                            <span className="absolute top-2 right-2 w-2 h-2 bg-primary rounded-full animate-pulse"></span>
                        </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-80 glass-card border-border p-0">
                        <div className="p-4 border-b border-border">
                            <h4 className="font-semibold">Notifications</h4>
                        </div>
                        <div className="p-2 space-y-1">
                            <DropdownMenuItem className="flex flex-col items-start gap-1 p-3 cursor-pointer hover:bg-foreground/5 rounded-lg focus:bg-foreground/5">
                ...
                <Link to="/profile" className="w-10 h-10 rounded-full p-[2px] cursor-pointer hover:scale-105 transition-transform bg-gradient-to-br from-primary via-accent to-primary animate-gradient bg-[length:200%_auto]">
                    <div className="w-full h-full rounded-full bg-black/50 backdrop-blur-md flex items-center justify-center border border-border">
